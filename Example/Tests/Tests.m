...
//  FMDBHelperTests
//
//  Created by lijingcheng on 09/26/2018.
...
describe(@"Tests", ^{
    // The application target is not the same as the test target, so...
    // [FMDBHelper setDataBaseName:@"demo.db"];
    [FMDBHelper setDataBasePath:[[[NSFileManager defaultManager] currentDirectoryPath] stringByAppendingPathComponent:@"demo.db"]];

    [FMDBHelper createTable:@"CREATE TABLE IF NOT EXISTS sys_user (id text PRIMARY KEY,name text,age integer,birthday text,dept text, dogs TEXT, nums TEXT)"];
    ...
    context(@"insert", ^{
        ...
        beforeEach(^{
            NSString *jsonPath = [[NSBundle bundleForClass:[self class]] pathForResource:@"demo" ofType:@"json"];
            keyValues = [NSJSONSerialization JSONObjectWithData:[NSData dataWithContentsOfFile:jsonPath] options:NSJSONReadingMutableLeaves error:nil];
        });
